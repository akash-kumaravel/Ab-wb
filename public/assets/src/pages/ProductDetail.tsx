...
const ProductDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Always start with constants as base
      const baseProducts = [...TRENDING_PRODUCTS, ...SPECIAL_OFFERS];
      console.log('Base products from constants:', baseProducts);
      ...
      try {
        ...
        // Use API products if available and not empty, otherwise use constants
        if (apiProducts && Array.isArray(apiProducts) && apiProducts.length > 0) {
          productsToUse = apiProducts;
          console.log('Using API products');
        } else {
          console.log('API returned no products, using constants');
          productsToUse = baseProducts;
        }
      } catch (error) {
        console.error('Error fetching from API:', error);
        console.log('Using constants as fallback');
        productsToUse = baseProducts;
      }
      
      console.log('Final products to search in:', productsToUse);
      setAllProducts(productsToUse);

      // Search for product
      console.log('Searching for product with slug:', productSlug);
      let found = findProductBySlug(productSlug || '', productsToUse);
      
      if (!found && productSlug) {
        // Try direct name match as last resort
        console.log('Slug not found, trying direct content match');
        found = productsToUse.find(p => p.name.toLowerCase() === productSlug.toLowerCase());
      }
      
      console.log('Final found product:', found);
